...
class Donor(User):
    def __init__(self, name: str, email: str) -> None:
        ...
        self.__donations_list: List[Donation] = (
            []
    ...
    def make_donation(
        self, amount: float, campaign: "FundraisingCampaign"
    ) -> "Donation":
    ...
    def donations(self) -> List["Donation"]:
        return (
            self.__donations_list.copy()
        )  # still encapsulation to not change the actual list
...
class FundraisingCampaign:
    def __init__(
        self, campaign_name: str, target_amount: float, manager: CampaignManager
    ) -> None:
        self.id = str(uuid.uuid4())
...
class Donation:
    def __init__(
        self, donor: Donor, amount: float, campaign: FundraisingCampaign
    ) -> None:
        self.id = str(uuid.uuid4())
        self.__donor = donor
    ...
    def get_remaining_amount(self) -> float:
        return max(
            0, self.__campaign.target_amount - self.__campaign.current_amount
        )  # just in case the target is filled
